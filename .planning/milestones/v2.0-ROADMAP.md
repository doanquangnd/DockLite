...
**Trạng thái:** Đã ship 2026-04-25  
**Phases:** 1–5  
**Tổng kế hoạch:** 13  
**Chứng thực:** `.planning/SECURITY-ATTESTATION.md`

Bản chụp roadmap đầy đủ tại thời điểm đóng milestone (tham chiếu lịch sử).
...
- SHIPPED — **v2.0 IPC Hardening (Zero-Trust)** — Hoàn tất 5/5 phase, có `SECURITY-ATTESTATION.md`
...
- [x] **Phase 2: Secrets at Rest** — Token chuyển từ settings.json plaintext sang Windows Credential Manager, migration tự động, endpoint rotate token
- [x] **Phase 3: Abuse Prevention and Audit** — Rate-limit per-IP REST+WS, giảm timeout mặc định, audit log structured cho endpoint nhạy cảm, log rotating file
- [x] **Phase 4: TLS Opt-in với TOFU** — Tự sinh self-signed cert ECDSA, trust-on-first-use fingerprint dialog, pin fingerprint vào Credential Manager, cert change warning
- [x] **Phase 5: Process Hardening** — Vá shell injection `bash -lc`, validation `wslUnixPath`, PID file thay `pkill -f`, test injection argv cho trivy/compose, viết SECURITY-ATTESTATION.md
...
Plans:
- [x] 02-01: WPF — `WindowsServiceApiTokenStore` (PasswordVault), `IServiceApiTokenStore` trong Core
- [x] 02-02: WPF — `AppSettingsStore` migration + `JsonIgnore` token; `AppFileLog` khi migrate
- [x] 02-03: Go + WPF — `POST /api/auth/rotate` + WPF + OpenAPI; `RequireBearerToken` dùng `MutableToken`
...
Plans:
- [x] 03-01: Go — `PerIPRateLimit` + `ExtendLongLivedRequestDeadlines` + `limits` 60s
- [x] 03-02: Go — `AuditSecuritySensitive` + `internal/audit` (stdout + file xoay)
...
Plans:
- [x] 04-01: Go — tự sinh self-signed ECDSA cert + load/reuse + TLS server setup
- [x] 04-02: WPF — HttpClient custom `ServerCertificateCustomValidationCallback` với fingerprint pin từ Credential Manager
- [x] 04-03: WPF — UI dialog TOFU và cert change, Settings toggle TLS + forget fingerprint
...
Plans:
- [x] 05-01: WPF — sửa shell injection `WslDockerServiceAutoStart` + validation `wslUnixPath` + test injection
- [x] 05-02: Go — PID file lifecycle script + argument injection guard cho trivy/compose + tests
- [x] 05-03: Docs — viết `SECURITY-ATTESTATION.md` tự attest từng ASVS control, đính kèm evidence
...
| 2. Secrets at Rest | 3/3 | Complete | 2026-04-23 |
| 3. Abuse Prevention and Audit | 2/2 | Complete | 2026-04-23 |
| 4. TLS Opt-in với TOFU | 3/3 | Complete | 2026-04-25 |
| 5. Process Hardening | 3/3 | Complete | 2026-04-25 |
