...
- WSL2 (Ubuntu khuyến nghị) và Go 1.25+ khi build service (theo `wsl-docker-service/go.mod`).

## Ghi chú phát hành nội bộ v2.0 (IPC Hardening)

- **Mạng và token:** service mặc định chỉ loopback; mở LAN bắt buộc token; rate-limit và audit cho thao tác nhạy cảm.
- **Token trên Windows:** lưu trong Credential Manager (không plaintext trong `settings.json`); có xoay token đồng bộ với service.
- **TLS tùy chọn:** bật `DOCKLITE_TLS_ENABLED` trên WSL để HTTPS tự ký; trên Cài đặt dùng `https://` và luồng **Kết nối và lưu pin TLS** (TOFU) trước khi gọi API qua HTTPS/WSS.
- **Spawn WSL:** DockLite gọi `wsl.exe` với `--cd` và script `bash` trực tiếp (tránh `bash -lc "cd ..."` cho lifecycle); script dừng/khởi động lại dùng **PID file** thay `pkill -f`.
- **Chứng thực:** `.planning/SECURITY-ATTESTATION.md` (self-attest 27 requirement v2.0).
...
```powershell
dotnet test DockLite.slnx -c Release; wsl --cd /home/<user>/path/wsl-docker-service bash scripts/test-go.sh
```

(Thay đường dẫn Unix bằng bản clone thật; thêm `-d <distro>` nếu không dùng distro mặc định.)
...
Trên **cùng máy**, mặc định dùng **HTTP** tới `http://127.0.0.1:17890/` là đủ khi service không bật TLS.

**TLS tùy chọn trong binary:** đặt `DOCKLITE_TLS_ENABLED=true` trong môi trường service (xem `wsl-docker-service/.env.example`). Khi bật, service phục vụ **HTTPS** (cert tự ký ECDSA, lưu dưới `~/.docklite/tls/`). Ứng dụng WPF: đặt Base URL `https://...`, dùng **Kết nối và lưu pin TLS** để TOFU và lưu fingerprint vào Credential Manager; WebSocket dùng `wss://` cùng pin.

Nếu cần truy cập từ **máy khác** trong LAN: không nên chỉ mở HTTP qua firewall; kết hợp **token** (`DOCKLITE_API_TOKEN`), **HTTPS** (TLS built-in hoặc reverse proxy), và giới hạn nguồn. Chi tiết mô hình đe dọa và WebSocket qua proxy: **`docs/docklite-lan-security.md`**.

## Cài đặt ứng dụng

Cấu hình lưu trong `%LocalAppData%\DockLite\settings.json` (ứng dụng đọc khi khởi động). **Token API** không nằm trong file JSON: đọc/ghi qua **Windows Credential Manager** (hồ sơ theo `ServiceApiTokenProfile`). Trên màn **Cài đặt** bạn có thể sửa rồi **Lưu**: `ServiceBaseUrl` (`http://` hoặc `https://` tới service Go trong WSL), ô token (đồng bộ với kho), tùy chọn **Bật TLS** / **Kết nối và lưu pin TLS** / **Quên pin TLS** khi dùng HTTPS, `HttpTimeoutSeconds` (30–600, mặc định 120), đường dẫn tới thư mục `wsl-docker-service`, tên distro WSL, và có bật **tự khởi động service WSL khi mở app** hay không.
...
2. Nếu **đã bật** «Tự khởi động service trong WSL» trong cài đặt (mặc định bật) **và** bước 1 không đạt: gửi lệnh tới `wsl.exe` (tuỳ chọn `-d <distro>`) với **`--cd` thư mục Unix** của `wsl-docker-service` rồi chạy `bash scripts/restart-server.sh` (script dùng **PID file** để dừng đúng tiến trình rồi chạy lại binary; xem phần Build service Go), rồi **chờ lặp** cho tới khi health + Docker info OK hoặc hết thời gian chờ (tab **Chờ và health**).
...
2. **Khởi động service WSL**: áp dụng tạm địa chỉ/timeout từ ô cho `HttpClient`, gửi `wsl.exe` chạy `bash scripts/run-server.sh` (cùng cơ chế `--cd` như tự khởi động), chờ `/api/health` tối đa khoảng **90 giây**; kết quả hiện trong ô trạng thái phía dưới (thành công hoặc hết thời gian / gợi ý).
...
1. `go build` **chưa** chạy server — trong **đúng** distro (ví dụ Ubuntu-22.04), tại thư mục project: `./bin/docklite-wsl` (hoặc `bash scripts/run-server.sh`). Giữ terminal mở; khi đó mới có process lắng nghe cổng (mặc định **`127.0.0.1:17890`** trong code; truy cập từ Windows qua `http://127.0.0.1:17890/` hoặc `https://...` nếu đã bật TLS trên service).
...
5. Nút **Khởi động service WSL** gọi `wsl.exe` với `--cd` tới thư mục dự án Unix rồi `bash scripts/run-server.sh` (không còn chuỗi `bash -lc "cd ..."` cho lifecycle). Nếu `go` chỉ có trong login shell, hãy build trong terminal WSL (`bash scripts/build-server.sh`) rồi dùng nút khởi động. `run-server.sh` **chỉ chạy binary** đã build (không gọi `go build`). Lỗi `go mod` / `go build` chỉ xuất hiện khi dùng `build-server.sh` (hoặc build tay), không phải từ `run-server.sh` khi binary đã có.
